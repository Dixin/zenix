...
test.describe('theme and navigation regressions', () => {
  ...
  test('navbar auth actions resolve to existing pages', async ({ page, isMobile }) => {
    ...
    await expect(page.getByRole('link', { name: 'View GitHub Repo' }).first()).toHaveAttribute('href', 'https://github.com/Dixin/CultureProtect');
...
test.describe('search modal regressions', () => {
  test('search results stay sharp and readable over blurred backdrop', async ({ page }) => {
    ...
    await page.locator('#search-input').fill('CultureProtect');
    ...
    await expect(result).toContainText(/CultureProtect/i);
...
test.describe('blog regressions', () => {
  test('blog detail renders author once with fallback avatar and balanced callouts', async ({ page }) => {
    await page.goto('/blog/CultureProtect/');

    const authorName = page.getByText('Dixin');
    await expect(authorName).toHaveCount(2);

    const headerAuthor = page.locator('article').getByRole('link', { name: /Dixin/ });
    ...
    const callout = page.getByText('The CultureProtect Philosophy').locator('..').locator('..');
  ...
  test('blog listing author metadata does not render broken image alt text', async ({ page }) => {
    ...
    await expect(cardMeta).toContainText('Dixin');
...
test.describe('seo regressions', () => {
  test('homepage exposes canonical, robots, open graph, and twitter metadata', async ({ page }) => {
    ...
    await expect(page.locator('link[rel="canonical"]')).toHaveAttribute('href', 'https://CultureProtect.com/');
    await expect(page.locator('link[rel="sitemap"]')).toHaveAttribute('href', '/sitemap.xml');
    await expect(page.locator('meta[name="robots"]')).toHaveAttribute('content', 'index, follow');
    await expect(page.locator('meta[property="og:title"]')).toHaveAttribute('content', 'Home | CultureProtect');
    await expect(page.locator('meta[property="og:type"]')).toHaveAttribute('content', 'website');
    await expect(page.locator('meta[property="og:url"]')).toHaveAttribute('content', 'https://CultureProtect.com/');
    await expect(page.locator('meta[property="og:image"]')).toHaveAttribute('content', 'https://CultureProtect.com/og-image.png');
    await expect(page.locator('meta[name="twitter:card"]')).toHaveAttribute('content', 'summary_large_image');
    await expect(page.locator('meta[name="twitter:site"]')).toHaveAttribute('content', '@dixnyan');
    await expect(page.locator('meta[name="twitter:image"]')).toHaveAttribute('content', 'https://CultureProtect.com/og-image.png');
  });

  test('article pages expose article dates and BlogPosting structured data', async ({ page }) => {
    await page.goto('/blog/CultureProtect/');

    await expect(page.locator('link[rel="canonical"]')).toHaveAttribute('href', 'https://CultureProtect.com/blog/CultureProtect/');
    ...
    expect(structuredData).toContain('"name":"Dixin"');
  });

  test('sitemap and robots are crawlable', async ({ page }) => {
    const sitemap = await page.goto('/sitemap.xml');
    expect(sitemap?.ok()).toBe(true);
    await expect(page.locator('body')).toContainText('https://CultureProtect.com/');
    await expect(page.locator('body')).toContainText('https://CultureProtect.com/blog/CultureProtect/');

    const robots = await page.goto('/robots.txt');
    expect(robots?.ok()).toBe(true);
    await expect(page.locator('body')).toContainText('User-agent: *');
    await expect(page.locator('body')).toContainText('Allow: /');
    await expect(page.locator('body')).toContainText('Sitemap: https://CultureProtect.com/sitemap.xml');
  });
});
